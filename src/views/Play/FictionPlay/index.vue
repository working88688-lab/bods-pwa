    ...
    <template v-else>
      <div class="page-content">
        ...
        <div class="chapter-content" v-else>
          ...
          <div v-else v-html="chapterHtml"></div>
        </div>
      </div>
...
import { comicById, chapterById, pageChapterByComicId } from '@/apis/comic'
      ...
      if (res && res.code === 0) {
        ...
        if (this.chapterData.preview) {
          ...
          if (contentRes && contentRes.status === 200) {
            this.chapterHtml = contentRes.data
            viewAdd({
              mediaId: comicId,
              mediaType: 10,
            })
...
.page-header {
  ...
  .right {
    ...
    .icon-share {
      background: url(../../../assets/images/share2.png) center / 100% no-repeat;
    }
  }
  .header-title {
    width: calc((100% - 88px));
...
::v-deep .page-content .chapter-content {
  font-size: 22px;
  line-height: 1.5;
  img {
    width: 100% !important;
