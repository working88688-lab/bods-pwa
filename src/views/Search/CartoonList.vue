...
export default {
  ...
  updated() {
    const LayoutDom = document.querySelector('.cartoon-grid')
    if (LayoutDom) {
        ...
        dataList = dataList.map(one => {
          if (one.type !== 'banner') {
            if (
              ...
              one.comicVo && one.comicVo.comicVo &&
              one.comicVo.comicVo.chapterCover &&
              one.comicVo.comicVo.chapterCoverHeight
            ) {
              one['coverHeight'] =
                (
                  one.comicVo.comicVo.chapterCoverHeight / one.comicVo.comicVo.chapterCoverWidth
                ).toFixed(2) *
                  100 +
                '%'
            } else if (
