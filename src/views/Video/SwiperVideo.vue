<template>
    <div class="player-content">
        <Loading v-if="isInit"/>
        <van-swipe v-else ref="videoSwiper" vertical :loop="isLoop" :show-indicators="false" @change="onSwipeChange">
            <van-swipe-item v-if="playVideo0">
                <div class="video-item" @click="onClickSwiper(0)">
                    <FullScreenPlayer
                        ...
                        :isCover="true"
                        :autoplay="false"
                        :isCanPlay="videoCanPlayer && swiperIndex === 0"
                        @ended="handleEnded"/>
            ...
            <van-swipe-item v-if="playVideo1">
                <div class="video-item" @click="onClickSwiper(1)">
                    <FullScreenPlayer
                        ...
                        :isCover="true"
                        :autoplay="false"
                        :isCanPlay="videoCanPlayer && swiperIndex === 1"
                        @ended="handleEnded"/>
            ...
            <van-swipe-item v-if="playVideo2">
                <div class="video-item" @click="onClickSwiper(2)">
                    <FullScreenPlayer
                        ...
                        :isCover="true"
                        :autoplay="false"
                        :isCanPlay="videoCanPlayer && swiperIndex === 2"
                        @ended="handleEnded"/>
...
<script>
import { listMediaBySearchType, listShortVideoRandom, searchMedia, purchase, getMediaById, viewAdd } from '@/apis/media'
import { uploaderSearch } from '@/apis/up'
...
export default {
    ...
    watch: {
        listenChange: {
            handler(value) {
                if (value && this.$route.path === '/swiperVideoList') {
                    this.init()
                }
            }
        }
    ...
    computed: {
        ...
        listenChange() {
            const { categoryId, videoId, pageNo, searchBy, searchType, keyword, upInfoId } = this
            return { categoryId, videoId, pageNo, searchBy, searchType, keyword, upInfoId }
        },
    },
    data() {
        return {
    ...
    methods: {
        init() {
            ...
            this.videoList = []
            this.page.pageNo = (this.pageNo ? this.pageNo : 1)
            this.swiperIndex = 0
            this.activedIndex = 0
            this.playVideo0 = null
            this.playVideo1 = null
            this.playVideo2 = null
            this.isLoop = false
            this.prevIndex = 0
            this.finished = false
            this.getVideoList(true, true)
        },
        activatedPlay() {
        ...
        onSwipeChange(index) {
            ...
            let direction = ''
            if (this.swiperIndex === 0 && this.prevIndex === 2) {
                direction = 'toTop'
                this.activedIndex++
            } else if (this.swiperIndex === 2 && this.prevIndex === 0) {
                direction = 'toBottom'
                this.activedIndex--
            } else {
                if (this.swiperIndex > this.prevIndex) {
                    direction = 'toTop'
                    this.activedIndex++
                } else {
                    direction = 'toBottom'
            ...
            this.prevIndex = this.swiperIndex
            // 预加载下一个视频
            if (direction === 'toTop') {
            ...
            if (this.videoList[this.activedIndex] && !this.videoList[this.activedIndex].preview) {
                viewAdd({
        ...
        async getVideoList(isRefresh, setVideoData) {
            if (this.finished) {
                return null
            }
            let res
            if (this.searchBy === 'random') {
                res = await listShortVideoRandom({
                    ...this.page,
            ...
            } else if (this.searchBy === 'categoryId') {
            ...
            } else if (this.searchBy === 'up') {
                res = await uploaderSearch({
                    ...this.page,
                    upInfoId: this.upInfoId,
                    mediaType: this.mediaType,
            ...
            this.videoCanPlayer = true
            if(res && res.code === 0) {
                if (res.data.dataList.length < this.page.pageSize) {
                    this.finished = true
                }
                if (isRefresh) {
                ...
                if (setVideoData) {
                    if (this.videoId) {
                        ...
                        if (this.searchBy === 'random') {
                            ...
                            const targetData = await getMediaById(params)
                            if (targetData && targetData.code === 0) {
                                this.videoList.unshift(targetData.data)
                            }
                        } else {
                            const index = _.findIndex(this.videoList, one => one.mediaId === this.videoId)
                            this.videoList = this.videoList.slice(index)
                            if (this.videoList.length - this.activedIndex <= 5 && !this.finished) {
                                this.page.pageNo++
                                return this.getVideoList(false, true)
                            }
                        }
                    } else {
                        this.activedIndex = 0
                    ...
                    if (this.videoList[this.activedIndex]) {
                        this.playVideo0 = this.videoList[this.activedIndex]
                    }
                    if (this.videoList[this.activedIndex + 1]) {
                        this.playVideo1 = this.videoList[this.activedIndex + 1]
                    }
                    if (this.videoList[this.activedIndex + 2]) {
                        this.playVideo2 = this.videoList[this.activedIndex + 2]
                    }
                    if (this.playVideo0 && !this.playVideo0.preview) {
                        viewAdd({
        ...
        handleAddComment() {
            this.rowVideo.commentCount = this.rowVideo.commentCount + 1
