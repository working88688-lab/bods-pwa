<template>
  <div class="page-container flex-container">
    <Header title="关注列表" textColor="#000" @onClickLeft="onBack" />
      ...
      <van-pull-refresh v-else v-model="refreshing" @refresh="onRefresh">
        ...
        >
          <div class="data-item" v-for="(data, idx) in dataList" :key="idx">
            ...
            <div class="flow-box">
              <span class="btn-flowed" @click="onFollow(data)">已关注</span>
            </div>
          </div>
        </van-list>
      </van-pull-refresh>
...
import { followedPage } from '@/apis/user'
...
export default {
  ...
  methods: {
    ...
    async getDataList(done, isRefresh) {
      ...
      this.loading = true
      const res = await followedPage(params).catch(() => {
        this.isInit = false
        done && done()
        this.loading = false
      })
    ...
    async onFollow(data) {
      ...
      if (res && res.code === 0) {
        this.dataList = this.dataList.filter(one => one.userId !== data.userId)
        this.$toast('取消成功')
      }
    },
  },
