<template>
  <div>
    <div class="sticky-top" v-if="!isRecom" v-sticky="{ zIndex: 99, stickyTop: -1, disabled: false }" @touchstart.stop
      ...
      <FilterComponent showStatus showPayType showSort @onFilter="handleFilter"
        @onChangeListLayout="handleChangeListLayout" />
    </div>
        ...
        <div class="recom-list" v-if="isRecom">
          <van-list v-model:loading="loading" :finished="finished" finished-text="没有更多了" @load="onLoad">
            ...
            <div class="recom-item" :id="'recom-item-' + idx" v-for="(recom, idx) in recomList" :key="idx">
              <BannerItem v-if="recom.type === 'banner'" :key="'banner' + idx" :bannerData="recom.data" />
              <template v-else>
        ...
        <template v-else>
          ...
          <van-list v-else v-model:loading="loading" :finished="finished" finished-text="没有更多了" @load="onLoad">
            <div :class="'masonry-grid masonry-grid-' + secondCategoryId">
              ...
              <div class="grid-item" :style="{ width: gridItemStyle }" v-for="(data, idx) in dataList" :key="idx">
        ...
        const resultData = res.data.dataList.map(one => {
          ...
          one.comicList &&
            one.comicList.map((two, m) => {
              if (
                two.comicVo &&
                two.comicVo.comicCover &&
                two.comicVo.coverHeight
              ) {
                two['coverHeight'] =
                  (two.comicVo.coverHeight / two.comicVo.coverWidth).toFixed(
                    2
                  ) *
                  100 +
                  '%'
                ...
                two['coverHeight'] =
                  (
                    two.chapterVo.coverHeight / two.chapterVo.coverHeight
                  ).toFixed(2) *
                  100 +
                  '%'
              } else {
